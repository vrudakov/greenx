...
contract GreenX is Owner {
    ...
    string public constant symbol = "GRX";
    uint public constant decimals = 18;
    uint256 constant public totalSupply = 500000000 * 10 ** 18; // 500 mil tokens will be supplied
    ...
    uint256 public constant salesAllocation = 300000000 * 10 ** 18; // 300 mil tokens allocated for sales
    uint256 public constant bonusAllocation = 20000000 * 10 ** 18; // 20 mil tokens allocated for token sale bonuses
    uint256 public constant advPartnersAllocation = 50000000 * 10 ** 18; // 50 mil tokens allocated for partners and advisor
    uint256 public constant reservedAllocation = 50000000 * 10 ** 18; // 50 mil tokens allocated for reserved, bounty campaigns, ICO partners, and bonus fund
    uint256 public constant founderAllocation = 50000000 * 10 ** 18; // 50 mil tokens allocated for founders
    uint256 public constant teamAllocation = 80000000 * 10 ** 18; // 80 mil tokens allocated for team
    ...
    event ModifyWhiteList(address investorAddress, string status);  // Add or remove investor's address to or from white list
    event ModifyPrivateList(address investorAddress, string status);  // Add or remove investor's address to or from private list
    ...
    constructor (address payable _walletAddr, address _adminAddr, address _portalAddr) public Owner(msg.sender) {
    ...
    function addToWhitelist(address[] calldata _investorAddrs) external isActive onlyOwnerOrAdminOrPortal returns(bool) {
        for (uint256 i = 0; i < _investorAddrs.length; i++) {
            ...
            emit ModifyWhiteList(_investorAddrs[i], "added");
        }
        return true;
    }
    
    function removeToWhitelist(address[] calldata _investorAddrs) external isActive onlyOwnerOrAdminOrPortal returns(bool) {
        for (uint256 i = 0; i < _investorAddrs.length; i++) {
            whiteList[_investorAddrs[i]] = false;
            emit ModifyWhiteList(_investorAddrs[i], "removed");
        }
        return true;
    }

    // Modify private list
    function addPrivateInvestor(address[] calldata _investorAddrs) external isActive onlyOwnerOrAdminOrPortal returns(bool) {
        for (uint256 i = 0; i < _investorAddrs.length; i++) {
            privateList[_investorAddrs[i]] = true;
            emit ModifyPrivateList(_investorAddrs[i], "added");
        }
        return true;
    }
    
     function removePrivateInvestor(address[] calldata _investorAddrs) external isActive onlyOwnerOrAdminOrPortal returns(bool) {
        for (uint256 i = 0; i < _investorAddrs.length; i++) {
            privateList[_investorAddrs[i]] = false;
            emit ModifyPrivateList(_investorAddrs[i], "removed");
        }
        return true;
    }
    ...
    function revokeTokens(address payable _noneKycAddr, uint256 _transactionFee) external onlyOwnerOrAdmin {
    ...
    // Modify wallet
    function changeWallet(address payable _newAddress) external onlyOwner {
    ...
    // Allocate tokens for founder vested gradually
    ...
    // Remaining tokens for sales will be locked by contract
    ...
    function getCurrentState() public view returns(uint256) {
        if (saleState == IN_1ST_ICO) {
            if (now > icoStartTime + 12 minutes) {
                return IN_3RD_ICO;
            }
